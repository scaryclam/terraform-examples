...
resource "aws_vpc_peering_connection" "target-peer" {
    # The targets VPC owner ID (account number). In this case the account happens
    # to be the same for target and source as we're not working accross accounts.
    # See the example in aws/vpc/multi-account-peered for cross account peering
