resource "aws_launch_configuration" "eks-example-launch-conf" {
  ...
  # Image IDs can be found just below https://docs.aws.amazon.com/eks/latest/userguide/getting-started-console.html#eks-launch-workers
...
resource "aws_autoscaling_group" "eks-example-asg" {
  ...
  tag {
    key = "kubernetes.io/cluster/${aws_eks_cluster.eks-cluster-1.name}"
    value = "owned"
